...
export async function runSidebarSwitchBench(): Promise<BenchResult[]> {
	...
	const context = await browser.newContext({
		...
		// The rail tabs are wrapped in animated base-ui tooltips. Asking for
		// reduced motion keeps the trigger from jittering under the pointer so
		// the click lands without a long actionability wait (headless never
		// settles the tooltip "stable" otherwise).
	...
	for (let i = 0; i < ITERATIONS; i += 1) {
		...
		try {
			// `force: true` skips Playwright's actionability/stability gate (the
			// tooltip animation otherwise keeps the trigger from settling
			// "stable" in headless). The `waitForSelector` below still asserts
			// the switch happened, so a no-op click fails the iteration rather
			// than recording a bogus sample.
			await page.click(`#${railId}`, { force: true });
	...
	if (samples.length === 0) {
		console.warn("bench: sidebar-switch — zero usable samples, skipping.");
		return [
			{
				name: "sidebar-switch",
				meanMs: Number.NaN,
