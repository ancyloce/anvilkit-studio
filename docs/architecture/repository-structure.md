...
- `check:no-headless-import` (`packages/runtime/core/scripts/check-no-headless-import.mjs`, part of `check:all`) forbids shipped `src/` from importing any `@anvilkit/*` package outside the `contracts`/`ui`/`utils` allowlist.
...
- Per-subpath API snapshots (`check:api-snapshot-*` for `.`, `runtime`, `config`, `i18n`, `icu`, `react`, `templates`) gate each boundary's public surface independently, so a change to one subpath cannot silently alter another.

**Split trigger (explicit).** Do **not** split `@anvilkit/core` now — splitting trades the "prefer boring" and public-API-stability rules for speculative benefit while the fixed release group and single maintainer absorb the coupling cost. Extract the plugin-contract/runtime surface (`./runtime` + `./config` + `./types`) — or, symmetrically, the studio chrome — into an independently versioned package **only when one of these concrete conditions holds**:
