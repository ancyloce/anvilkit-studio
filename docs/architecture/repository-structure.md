...
- `check:no-headless-import` (`packages/runtime/core/scripts/check-no-headless-import.mjs`, part of `check:all`) forbids shipped `src/` from importing any `@anvilkit/*` package outside the `contracts`/`schema`/`ui`/`utils` allowlist — with one directory-scoped exception (DD-0019 §6.4, PLAN-0020 CORE-P0-016): the React-free editor engine `src/editor/` (the `./editor` subpath) may additionally import `ir`, and may never import `ui`. Core therefore carries `@anvilkit/schema` as a production dependency since the visual-editor Phase 0.
	- **`@anvilkit/schema` is allowlisted across all of `src/` (corrected 2026-08-12).** The gate previously confined it to `src/editor/`, which the codebase had outgrown: six shipped modules under `src/puck/`, `src/style-compiler/`, and `src/migrations/` import `@anvilkit/schema/editor` to validate *declared document data* (appearance, interactions, bindings, design system, component library) at the write/read boundary, leaving the gate red at `HEAD` and pushing new consumers into re-export laundering. The restriction was never load-bearing for `schema`: it is a foundation leaf whose only workspace dependency is `@anvilkit/contracts` (`packages/foundation/schema/package.json`), so `core -> schema` is a plain downward `runtime -> foundation` edge and the inversion this gate guards is structurally impossible. `@anvilkit/ir` and `@anvilkit/validator` remain forbidden outside `src/editor/`, and capability/extension packages remain forbidden everywhere.
