...
import { localizedDisplayName } from "@/lib/collab-name-pools";
import { useDemoLocale } from "@/lib/i18n/client";
...
export function useDemoIdentity(
	...
	// Active demo language → drives the locale-aware fresh-visitor name. Safe
	// here: `<DemoI18nProvider>` wraps the whole app from the root layout, so
	// every editor mount has the context.
	...
	useEffect(() => {
		...
		const id = generateId();
		const displayName = localizedDisplayName(locale, id);
		...
		setReady(true);
	}, [enabled, options.peerOverride, locale]);
