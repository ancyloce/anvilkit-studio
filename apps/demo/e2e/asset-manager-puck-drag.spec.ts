...
test("Puck drag: upload via sidebar, insert via tile click, export resolves the data URL", async ({
	...
	// Step 3 — publish to commit the change to `publishedData`. The chrome's
	// PublishPanel is a popover: click the "Publish" trigger to open it, then
	// the "Publish to live" action (which now hands the live document to the
	// host's onPublishClick).
	await page
...
async function runExportFromPanel(
	...
	// Drive the `?e2e=puck-drag` export hook directly. It IS the chrome's
	// unified `onExport(formatId)` handler (the AnvilKit `<PublishPanel>`
	// Export submenu calls the same function), but it writes results to
	// `window.__puckExports` instead of triggering a download. We avoid
	// clicking labelled buttons because the demo ALSO renders standalone
	// "Download HTML" / "Export React" buttons whose handlers download
	// without populating `__puckExports` — a name-regex match there would
	// silently skip the assertion target.
