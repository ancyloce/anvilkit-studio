...
async function activateModule(page: Page, key: ModuleKey): Promise<void> {
	...
	await expect(
		page.getByTestId(MODULE_TESTID[key]),
...
test.describe("StudioSidebar — four-module switcher", () => {
	...
	test("G1.1 — module switch + reload persistence (AC-SHL-1, AC-SHL-3)", async ({
		...
		await expect(
			page.locator(`#${RAIL_TAB_ID.image}`),
		...
		await expect(
			page.getByTestId(MODULE_TESTID.image),
	...
	test("G1.4 — insert search filters list and shows empty state (AC-INS-1)", async ({
		...
		await expect(
			page.getByTestId("ak-insert-empty-search"),
		).toBeVisible({ timeout: 3_000 });

		// Clear → empty state vanishes; sectioned layout returns.
		await searchInput.fill("");
		await expect(page.getByTestId("ak-insert-empty-search")).toHaveCount(
			0,
			{ timeout: 3_000 },
		);
	});
	...
	test("G1.6 — layer pages list populated; route badge visible (AC-LYR-1, 3, 4)", async ({
		...
		await expect(
			page.getByTestId("ak-layer-page-row-home"),
		).toBeVisible();
	...
	test("G1.8 — layer `+` quick-add inserts a node (AC-LYR-6)", async ({
		...
		const entry = popup.getByTestId(
			"ak-layer-quickadd-plugin:demo-add-hero",
		);
	...
	test("G1.11 — image overflow menu opens; Copy URL writes to clipboard (AC-IMG-8)", async ({
		...
		// Seed an asset.
		await page
			.getByTestId("ak-image-upload-input")
			.setInputFiles({
		...
		await tile.hover();
		const overflowTrigger = tile.locator(
			'[data-testid^="ak-image-overflow-"]',
		);
		...
		for (const label of ["Rename", "Replace", "Copy URL", "Delete"]) {
			await expect(
				popup.getByRole("menuitem", { name: label }),
			).toBeVisible();
		}
	...
	test("G1.12 + G1.13 — text snippet rows reflect selection state and toast on no-op (AC-TXT-6)", async ({
		...
		const firstRow = page
			.locator('[data-testid^="ak-text-snippet-"]')
			.first();
	...
	test("G1.15 — `studio.tab.insert` override resolves through alias (AC-SHL-4)", async ({
		page,
	}) => {
		const overrides = encodeURIComponent(
			JSON.stringify({ "studio.tab.insert": "Custom Insert" }),
		);
		await gotoEditor(page, { search: `?messageOverrides=${overrides}` });

		// Rail tab labels read `studio.module.insert.name`; with the
		...
		await expect(
			page.locator(`#${RAIL_TAB_ID.insert}`),
		).toHaveAttribute("aria-label", "Custom Insert");
	});
	...
	test("G6 — RTL: rail flips to the right edge (PRD §13)", async ({
		page,
	}) => {
		await gotoEditor(page);
		await page.evaluate(() => {
