...
export async function publish(
	...
	const input = body as { id?: string; slug?: string; data: EditorPageData };
	const record = await storage.publish({
		id: input.id,
		slug: input.slug,
		data: input.data,
	});
	// NOTE: the studio copy of this handler also records a server-side
	// `page_published` analytics event here. This app has no analytics
	// wiring yet (plan 0036 P0 scope), so that call is deliberately absent
	// rather than stubbed — reinstate it with the analytics package.
	return { status: 200, body: apiSuccess(record) };
}
