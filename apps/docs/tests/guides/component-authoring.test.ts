...
describe("§1 — The contract", () => {
	...
	it("componentConfig exposes the required Puck surface", () => {
		...
		// buildConfig() and the static `fields` export each call buildFields(),
		// so they are deep-equal but no longer the same reference.
		expect(buttonConfig.fields).toStrictEqual(fields);
