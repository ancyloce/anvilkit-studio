...
> **Alpha（`0.1.11`）。** 在 `v1.0` 之前公共接口仍可能变动。CI 中强制执行的包体积预算：headless 初始分块 ≤ 17 KB gzip，UI 初始分块 ≤ 12 KB gzip，Unsplash 子路径 ≤ 4 KB gzip。
...
非可选的 peer：`react >=19.2.0`、`react-dom >=19.2.0`、`@puckeditor/core ^0.23.0`。
...
| `onAssetDeleted`            | `(asset: UploadResult) => void \| Promise<void>` | none    | 当资源经由默认来源被删除或其 URL 被替换时触发的生命周期 hook；在此释放旧的后端对象（`blob:` URL 会被自动撤销）。 |

`unsplash.proxyEndpoint` 是 API 基础路径，而不是仅用于搜索的路由。provider
会将所有 Unsplash 路径追加到该地址，包括强制要求的
`/photos/:id/download` 下载触发器，因此代理必须转发这些路径，并在服务端注入
`Client-ID`。相对基础路径（如 `/api/unsplash`）和绝对基础路径（如
`https://example.com/api/unsplash`）均受支持。

设置 `unsplash.rehostOnPick: true` 后，选中的图片会先下载并通过已配置的上传器，
再插入编辑器。生成的本地资源会保留 Unsplash 署名元数据，并继续触发强制下载
追踪；默认行为仍是符合规范的 Unsplash 热链接。
...
| `AssetManagerUI`      | `{ uploader, registry, categories?, facets?, onFilterChange? }`（复合） |

已配置的分类和分面控件会按需加载。本地定义使用 AND 组合进行过滤；provider 分类和
远程分面会保留在发出的 `AssetFilter` 中，由数据源负责路由。
...
6. **锁定包体积。** `.size-limit.mjs` 将 headless 初始分块保持在 17 KB gzip 以下、UI 初始分块保持在 12 KB 以下。CI 对两者都设置门禁。
