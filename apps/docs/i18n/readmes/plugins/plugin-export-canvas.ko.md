...
peer 의존성: `@puckeditor/core ^0.23.0`(필수); `react`와 `react-dom`(`>=19.0.0`)은 **선택적** peer입니다——이 플러그인은 헤드리스이며 호스트가 React 트리를 공유하는 경우에만 필요합니다. `@anvilkit/canvas-core`는 직접 의존성으로 동봉되며 기반이 되는 직렬화기를 제공합니다.
