...
> **Alpha(`0.1.11`).** `v1.0` 이전에는 공개 표면이 아직 바뀔 수 있습니다. CI에서 강제되는 번들 예산: headless 초기 청크 ≤ 17 KB gzip, UI 초기 청크 ≤ 12 KB gzip, Unsplash 서브경로 ≤ 4 KB gzip.
...
선택적이지 않은 peer: `react >=19.2.0`, `react-dom >=19.2.0`, `@puckeditor/core ^0.23.0`.
...
| `onAssetDeleted`            | `(asset: UploadResult) => void \| Promise<void>` | none    | 기본 소스를 통해 에셋이 삭제되거나 URL이 교체될 때 발화하는 라이프사이클 hook. 여기서 이전 백엔드 객체를 해제하세요(`blob:` URL은 자동으로 취소됩니다). |

`unsplash.proxyEndpoint`는 검색 전용 라우트가 아니라 API 기본 경로입니다. provider는
필수 `/photos/:id/download` 다운로드 트리거를 포함한 모든 Unsplash 경로를 이
기본 경로에 추가합니다. 따라서 프록시는 해당 경로를 전달하고 서버 측에서
...
interface CreateStudioAssetSourceOptions {
  ...
  readonly maxConcurrentUploads?: number; // 양의 정수, 기본 3
}
```
...
`StudioAssetSource.upload(files)`는 최대 `maxConcurrentUploads`(기본 3)개의 업로드를 병렬로 실행합니다. 값은 양의 정수여야 하며 `NaN`, 무한대, 0, 음수 또는 소수를 전달하면 소스를 만들 때 `RangeError`가 발생합니다. 파일별 실패는 리스너를 통해 `error` 엔벨로프로 드러납니다. 반환된 promise는 성공한 부분집합으로 resolve되며 **던지지 않습니다**. 빠른 실패 시맨틱이 필요한 호스트는 `maxConcurrentUploads: 1`을 전달할 수 있습니다. 어댑터에서 온 `AbortError`는 배치 전체를 중단합니다 — 대기 중인 파일은 스케줄되지 않습니다.
...
6. **번들을 잠그세요.** `.size-limit.mjs`는 headless 초기 청크를 17 KB gzip 미만, UI 초기 청크를 12 KB 미만으로 유지합니다. CI가 둘 다에 게이트를 겁니다.
