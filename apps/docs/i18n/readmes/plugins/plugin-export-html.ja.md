...
> **Alpha（`0.1.8`）。** パッケージのインターフェイスは実装済みでテスト済みですが、発行される HTML/CSS の契約は `1.0.0` までに変化する可能性があります。
...
非オプションの peer：`react >=19.0.0`、`react-dom >=19.0.0`、`@puckeditor/core ^0.22.1`。ランタイム依存は `@anvilkit/core` のみです。
