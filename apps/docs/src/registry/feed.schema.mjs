/**
 * Zod schema for the Anvilkit marketplace registry feed (the single source of
 * truth for the /marketplace catalog and `anvilkit add <slug>`).
 *
 * Plain-JS (.mjs) port of apps/docs/src/registry/feed.schema.ts — the TS type
 * exports are compile-time only and dropped here so the generator can run under
 * plain `node` without jiti/tsx.
 */
import { z } from "zod";
