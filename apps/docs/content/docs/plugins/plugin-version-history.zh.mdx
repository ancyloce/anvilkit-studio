...
<div className="anvilkit-plugin-meta">
	<code>@anvilkit/plugin-version-history</code> <span>·</span> <span>v0.1.8</span> <span>·</span> <span>历史</span> <span>·</span> <span>顶栏操作</span>
</div>
...
| 导出 | 类型 |
|--------|------|
| `createPermissionDeniedError` | 工厂函数 |
| `createVersionHistoryPlugin` | 工厂函数 |
| `DiffApplyError` | 组件 / 类 |
| `VersionHistoryError` | 组件 / 类 |
| `applyDiff` | 函数 |
| `checkRestoreConflict` | 函数 |
| `diffIR` | 函数 |
| `inMemoryAdapter` | 函数 |
| `isIRDiff` | 函数 |
| `localStorageAdapter` | 函数 |
| `normalizeVersionHistoryExport` | 函数 |
| `planRetention` | 函数 |
| `prepareRestore` | 函数 |
| `querySnapshots` | 函数 |
| `runRetention` | 函数 |
...
| `MaybePromise` | 类型 |
| `PeerInfo` | 类型 |
| `PrepareRestoreInput` | 类型 |
| `PresenceCursor` | 类型 |
| `PresenceSelection` | 类型 |
| `PresenceState` | 类型 |
| `RestoreConflict` | 类型 |
| `RestoreConflictInput` | 类型 |
| `RetentionAdapter` | 类型 |
| `RetentionPlan` | 类型 |
| `RetentionPlanOptions` | 类型 |
| `RetentionPolicy` | 类型 |
| `RetentionResult` | 类型 |
| `RunRetentionOptions` | 类型 |
| `SnapshotAdapter` | 类型 |
| `SnapshotAdapterPresence` | 类型 |
| `SnapshotMeta` | 类型 |
| `SnapshotMetaPatch` | 类型 |
| `SnapshotPage` | 类型 |
| `SnapshotQuery` | 类型 |
| `Unsubscribe` | 类型 |
| `VersionHistoryErrorCode` | 类型 |
| `VersionHistoryExport` | 类型 |
| `VersionHistoryExportEntry` | 类型 |
| `VersionHistoryImportOptions` | 类型 |
| `VersionHistoryOperation` | 类型 |
