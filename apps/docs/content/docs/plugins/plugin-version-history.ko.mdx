...
description: "Anvilkit Version History 플러그인."
---

<div className="anvilkit-plugin-meta">
	<code>@anvilkit/plugin-version-history</code> <span>·</span> <span>v0.1.7</span> <span>·</span> <span>기록</span> <span>·</span> <span>헤더 액션</span>
</div>
...
<Studio
	puckConfig={config}
	plugins={[createVersionHistoryPlugin({ /* 옵션 — 아래 가이드 참조 */ })]}
/>;
```
...
| 익스포트 | 종류 |
|--------|------|
| `createVersionHistoryPlugin` | 팩토리 |
| `DiffApplyError` | 컴포넌트 / 클래스 |
| `VersionHistoryError` | 컴포넌트 / 클래스 |
| `applyDiff` | 함수 |
| `diffIR` | 함수 |
| `inMemoryAdapter` | 함수 |
| `localStorageAdapter` | 함수 |
| `summarizeDiff` | 함수 |
| `CreateVersionHistoryPluginOptions` | 타입 |
| `IRDiff` | 타입 |
| `IRDiffOp` | 타입 |
| `IRDiffSummary` | 타입 |
| `LocalStorageAdapterOptions` | 타입 |
| `MaybePromise` | 타입 |
| `PeerInfo` | 타입 |
| `PresenceCursor` | 타입 |
| `PresenceSelection` | 타입 |
| `PresenceState` | 타입 |
| `SnapshotAdapter` | 타입 |
| `SnapshotAdapterPresence` | 타입 |
| `SnapshotMeta` | 타입 |
| `Unsubscribe` | 타입 |
| `VersionHistoryErrorCode` | 타입 |
