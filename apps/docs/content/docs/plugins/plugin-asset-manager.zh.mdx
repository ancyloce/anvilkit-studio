---
title: "Asset Manager"
description: "Anvilkit Asset Manager 插件。"
---

<div className="anvilkit-plugin-meta">
	<code>@anvilkit/plugin-asset-manager</code> <span>·</span> <span>v0.1.11</span> <span>·</span> <span>资源</span> <span>·</span> <span>顶栏操作</span>
</div>
...
<Studio
	puckConfig={config}
	plugins={[createAssetManagerPlugin({ /* 选项 —— 参见下方指南 */ })]}
/>;
```
...
| 导出 | 类型 |
|--------|------|
| `createAssetManagerPlugin` | 工厂函数 |
| `createAssetReference` | 工厂函数 |
| `createAssetRegistry` | 工厂函数 |
| `createIRAssetResolver` | 工厂函数 |
| `createStudioAssetSource` | 工厂函数 |
| `AssetResolutionError` | 组件 / 类 |
| `AssetSourceError` | 组件 / 类 |
| `AssetValidationError` | 组件 / 类 |
| `dataUrlUploader` | 函数 |
| `getAssetRegistry` | 函数 |
| `getRequiredCsp` | 函数 |
| `inferAssetKind` | 函数 |
| `inferStudioAssetKind` | 函数 |
| `inMemoryUploader` | 函数 |
| `isResumableAdapter` | 函数 |
| `parseAssetReference` | 函数 |
| `resolveAssets` | 函数 |
| `resolveFolderId` | 函数 |
| `uploadAsset` | 函数 |
| `validateUploadResult` | 函数 |
| `ASSET_MANAGER_ERROR_EVENT` | 常量 |
| `ASSET_MANAGER_UPLOADED_EVENT` | 常量 |
| `AssetCategory` | 类型 |
| `AssetDataSource` | 类型 |
| `AssetDeletedHook` | 类型 |
| `AssetFacetDefinition` | 类型 |
| `AssetFacetOption` | 类型 |
| `AssetFilter` | 类型 |
| `AssetFolder` | 类型 |
| `AssetKind` | 类型 |
| `AssetListPage` | 类型 |
| `AssetManagerErrorEvent` | 类型 |
| `AssetManagerEventMap` | 类型 |
| `AssetManagerEventName` | 类型 |
| `AssetManagerOptions` | 类型 |
| `AssetManagerUploadedEvent` | 类型 |
| `AssetMeta` | 类型 |
| `AssetRegistry` | 类型 |
| `AssetRegistryListener` | 类型 |
| `AssetResolutionErrorCode` | 类型 |
| `AssetSearchOptions` | 类型 |
| `AssetSearchPage` | 类型 |
| `AssetSort` | 类型 |
| `AssetSortField` | 类型 |
| `AssetSourceCapabilities` | 类型 |
| `AssetSourceErrorCode` | 类型 |
| `AssetSourceId` | 类型 |
| `AssetSourceProvider` | 类型 |
| `AssetSourceStatus` | 类型 |
| `AssetTheme` | 类型 |
| `AssetTransform` | 类型 |
| `CreateIRAssetResolverOptions` | 类型 |
| `CreateStudioAssetSourceOptions` | 类型 |
| `DataUrlUploaderOptions` | 类型 |
| `FolderId` | 类型 |
| `FolderOptions` | 类型 |
| `JsonValue` | 类型 |
| `ParsedAssetReference` | 类型 |
| `PartTag` | 类型 |
| `PersistedUploadSession` | 类型 |
| `ReplacePayload` | 类型 |
| `RequiredCsp` | 类型 |
| `RequiredCspOptions` | 类型 |
| `ResumableUploadAdapter` | 类型 |
| `ResumableUploadConfig` | 类型 |
| `S3CspOptions` | 类型 |
| `S3MultipartCspOptions` | 类型 |
| `TransformResolver` | 类型 |
| `UnsplashSourceOptions` | 类型 |
| `UnsplashTheme` | 类型 |
| `UnsplashThemeConfig` | 类型 |
| `UploadAdapter` | 类型 |
| `UploadAdapterOptions` | 类型 |
| `UploadPart` | 类型 |
| `UploadResult` | 类型 |
| `UploadSession` | 类型 |
| `UploadSessionStore` | 类型 |
| `ValidateUploadResultOptions` | 类型 |
