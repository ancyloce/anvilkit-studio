...
const read = () =>
	page.evaluate(() => {
		...
		const g = document.querySelector('[class*="steps"]');
		return {
			st: ST ? Math.round(ST.getAll()[0].scroll()) : null,
			winY: Math.round(window.scrollY),
			card:
				g && g.firstElementChild
					? +getComputedStyle(g.firstElementChild).opacity
					: null,
		};
	});
...
console.log(
	JSON.stringify(
		{ A_scrollTo: a, B_dispatchEvent: b, C_wheel: c, D_end: d },
		null,
		1,
	),
);
