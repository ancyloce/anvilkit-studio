...
/**
 * The visual-regression suite (CORE-P1A-007 / CORE-P2-009H).
 *
 * Every functional project ignores it and only the opt-in
 * `editor-visual` project runs it. Two reasons, both structural:
 * baselines are CI-generated by policy (plan EP-20 risk row — WSL2
 * headless screenshot capture is verified broken), and a screenshot
 * assertion without a committed baseline fails, so leaving it in the
 * default project would red every PR rather than the one job built to
 * expect and produce baselines.
 */
const VISUAL_SPEC = /editor\/visual-regression\.spec\.ts/;
	...
	projects: [
		{
			...
			// Pixel assertions belong to `editor-visual` alone — see the
			// VISUAL_SPEC comment above.
		...
		// geometry assertions repeated at a high-density factor.
		{
			...
			// A DPR-2 shot would need its own baseline set; the DPR axis is
			// a geometry claim, not a pixel one.
			testIgnore: VISUAL_SPEC,
		},
		// Visual regression certification (CORE-P1A-007, CORE-P2-009H).
		// Its own project, and opt-in, because baselines are CI-generated
		// by policy (EP-20 risk row: WSL2 screenshot capture is broken) —
		// so a missing baseline must fail a job built to expect it, not
		// every PR. `deviceScaleFactor: 1` and a pinned viewport keep the
		// committed PNGs reproducible.
		...(process.env.ANVILKIT_E2E_VISUAL === "1"
			? [
					{
						name: "editor-visual",
						use: {
			...
			? [
					{
						...
						testMatch: /editor\/.*\.spec\.ts/,
						// Per-engine rasterization differs; baselines are
