...
Requires Node >= 22.13 and pnpm 12.3.4 (pinned via `packageManager`).
...
| Workspace | pnpm 12.3.4 + Turborepo 2 |
...
`.github/workflows/ci.yml` runs on every pull request (pnpm 12.3.4 / Node 22, submodules pulled recursively) and is split into path-aware jobs so changes scoped to a single leaf app skip unrelated work:
