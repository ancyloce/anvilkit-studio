...
Requires Node >= 22.13 and pnpm 11.20.0 (pinned via `packageManager`).
...
| Workspace | pnpm 11.20.0 + Turborepo 2 |
...
`.github/workflows/ci.yml` runs on every pull request (pnpm 11.20.0 / Node 22, submodules pulled recursively) and is split into path-aware jobs so changes scoped to a single leaf app skip unrelated work:
