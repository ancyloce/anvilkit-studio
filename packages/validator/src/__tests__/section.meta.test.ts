...
const baseReplacementNode = (meta: unknown): Record<string, unknown> => ({
	id: "new-1",
	type: "Hero",
	props: { title: "T" },
	meta,
...
describe("validateAiSectionPatch — meta cap enforcement", () => {
	...
	it("rejects oversize notes (>512 chars)", () => {
		...
		const metaIssue = result.issues.find((i) => i.path.includes("notes"));
	...
	it("rejects oversize owner (>256 chars)", () => {
		...
		expect(result.valid).toBe(false);
		expect(result.issues.find((i) => i.path.includes("owner"))).toBeDefined();
	});
	...
	it("rejects a non-semver version string", () => {
		...
		expect(result.valid).toBe(false);
		expect(result.issues.find((i) => i.path.includes("version"))).toBeDefined();
	});
	...
	it("rejects a non-boolean locked", () => {
		...
		expect(result.valid).toBe(false);
		expect(result.issues.find((i) => i.path.includes("locked"))).toBeDefined();
	});

	it("rejects an unknown meta key", () => {
		const result = validateAiSectionPatch(buildPatch({ foo: "bar" }), baseCtx);
		expect(result.valid).toBe(false);
		expect(result.issues.find((i) => i.code === "INVALID_NODE")).toBeDefined();
	});
	...
	it("validates meta on deeply-nested children", () => {
		...
		expect(result.valid).toBe(false);
		const issue = result.issues.find((i) => i.path.includes("notes"));
