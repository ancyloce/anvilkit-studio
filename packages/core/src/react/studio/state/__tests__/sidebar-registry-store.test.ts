...
const DESIGN_SYSTEM_PANEL: StudioDesignSystemPanel = {
	render: () => null,
};
...
describe("sidebar-registry-store", () => {
	...
	it("registerDesignSystemPanel last-write-wins; matching unregister clears", () => {
		const store = createSidebarRegistryStore();
		const off1 = store
			.getState()
			.registerDesignSystemPanel(DESIGN_SYSTEM_PANEL);
		expect(store.getState().designSystemPanel).toBe(DESIGN_SYSTEM_PANEL);

		const second: StudioDesignSystemPanel = { render: () => null };
		store.getState().registerDesignSystemPanel(second);
		expect(store.getState().designSystemPanel).toBe(second);
