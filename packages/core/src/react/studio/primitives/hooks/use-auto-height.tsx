...
export function useAutoHeight<T extends HTMLElement = HTMLDivElement>(
  ...
  React.useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      const next = measure();
      // Cancel a still-pending frame before scheduling a new one so
      // a burst of resize callbacks coalesces into a single
      // setHeight, and so cleanup can cancel the last frame.
    ...
    return () => {
      ro.disconnect();
      roRef.current = null;
      // Cancel any frame scheduled by the observer above so a
      // late setHeight cannot fire after unmount.
      if (rafRef.current !== null) {
        cancelAnimationFrame(rafRef.current);
        rafRef.current = null;
      }
    };
    // biome-ignore lint/correctness/useExhaustiveDependencies: animate-ui upstream uses caller-provided deps array
  }, deps);
