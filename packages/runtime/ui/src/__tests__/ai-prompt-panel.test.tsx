...
import { AiPromptPanel, type AiPromptPanelSelection } from "../ai-prompt-panel";
...
describe("<AiPromptPanel>", () => {
	...
	describe("section mode (1+ selection)", () => {
		...
		it("disables submit and shows guidance when onRegenerate is missing", () => {
			...
			expect(
				screen.getByTestId("ai-prompt-panel-diagnostics"),
			).toHaveTextContent(/no section regenerator|configure/i);
		});
